...
office_scene.py 는 무수정 엔진으로 쓴다 (사용자 지시 -- 새 파일로만).
이 모듈이 하는 일:

  1. 스캔 메시에 이미 구워져 있는 가구를 프로시저럴에서 뺀다
       기본값으로 꺼지는 것 (CLI 로 되살릴 수 있음):
           파티션 / 키보드 / 마우스(패드) / PC / 세로 모니터 / 연필꽂이
           (연필꽂이는 태스크 고정물이라 eval 이 자기 기본값으로 되켠다)
       토글이 없어 cfg 에서 떼는 것 (_BAKED):
           메인 모니터 3피스 + 단상 3피스
       남기는 것: 가운데+보조 책상 상판/다리 -- 벤치 전체의 "충돌" 담당
  2. 스캔 책상 USD 를 얹는다 (시각 전용, 충돌 없음).
     스캔 z0 = office z0 = 상판이라 두 면이 자동 일치, z-fighting 은
     --scan-lift (기본 2mm) 로 회피. 배치는 칸막이 기준 자동 정렬.
  3. 축 변환: 스캔 프레임은 칸막이가 +y, office 는 뒤쪽이 -x
     -> z 축 +90도 회전 (자동 배치는 yaw 부호를 따라간다)

태스크(마커/연필꽂이)와 로봇 리그, 카메라, 성공 판정은 office 그대로다.

확정 상수(아래 PROTOCOL 블록)가 벤치마크 정의의 정본이다 -- 값을 바꾸면
PROTOCOL 문자열을 올린다 (러너는 이 값들을 전달하지 않는다).
...
# 위임하므로 동작에는 필요 없다 (문서화 목적)
EGO_EYE = osc.EGO_EYE
EGO_TARGET = osc.EGO_TARGET


def __getattr__(name):
    """여기 없는 심볼은 전부 office_scene 으로 위임한다 (PEP 562).

    eval/preview 가 osc.MAX_ITEMS, osc.quat_axis 등 다수를 참조하므로
    이 모듈이 office_scene 의 완전한 대체물이 된다. 주의: 없는 이름의
    AttributeError 는 office_scene 명의로 뜬다 (디버깅 시 참고)."""
...
# cfg 속성명 규약에 의존한다 -- office 가 이름을 바꾸면 build 가 경고한다)
_BAKED = (
    "stand_base_main",
    "stand_col_main",
    "panel_main",
...
def srgb_to_linear(rgb):
    ...
    같은 sRGB 값으로 맞추려면 같은 변환을 태워야 렌더에서 만난다."""
    return tuple(
...
def add_scene_args(p):
    ...
    # 구운 가구 정책: "사용자 입력은 존중, 우리는 기본값만 바꾼다".
    # 스캔에 구워진 가구를 기본 꺼짐으로 깔되, CLI 로 되살릴 수 있다
    # (스캔과 겹쳐 보이는 것은 사용자 선택). 연필꽂이(holder)는 태스크
    # 고정물이라 eval 스크립트가 자기 set_defaults 로 다시 켠다.
    # region 은 eval, item_region 은 preview 의 인자라 둘 다 걸어둔다
    # (없는 쪽 키는 무해). 이 기본값 생존은 "eval 의 후속 set_defaults 가
    # 이 키들을 안 건드린다" 에 의존한다 -- build 가 effective 값을 로그로
    # 찍으므로 계약이 깨지면 즉시 드러난다.
    p.set_defaults(
        partition=0,
        pc=0,
        keyboard=0,
...
def _recolor(cfg, match_fn, lin, what):
    """조건에 맞는 책상 부품들의 diffuse 를 갈아끼우고 매칭 수를 검증한다."""
    hit = 0
...
def build(a, with_camera=True):
    ...
    # 조명: 스캔 버텍스컬러에는 촬영 조명이 구워져 있어 office 기본값이 과하다
    ...
    # 붙이고, 장축 중심은 wings 비대칭(왼 0.6/오른 0.4)의 벤치 중심에 둔다.
    # 수식은 yaw 부호를 따라간다 (+90/-90 외의 각도는 --scan-pos 수동)
    if a.scan_pos.strip():
    ...
    # 자산/배치로 돌았나" 를 사후 추적 가능하게 한다 (JSON 파싱 가능)
    eff = {
        "protocol": PROTOCOL,
        "scan_usd": a.scan_usd,

#!/usr/bin/env bash
# =============================================================================
# 사무실 마커 태스크 폐루프 평가 실행 (호스트에서 실행; 클라이언트는 컨테이너 안)
#
# 순서: 호스트에서 서버 먼저 (bimanual 서버 스크립트 재사용, CKPT 만 교체):
...
#   DRY_RUN=1 CLIENT_GPU=2 bash run_eval_office.sh  (서버 없이 관측 규약만 확인)
#
# 산출물: 컨테이너 /root/project/out/eval_office/<KST스탬프>/
#         (호스트 ~/project/gr00t_Isaacsim/out/eval_office/... 로 보임)
# =============================================================================
set -euo pipefail
...
SCRIPT_DIR="${SCRIPT_DIR:-/root/project/isaac_franka/envs/office}"

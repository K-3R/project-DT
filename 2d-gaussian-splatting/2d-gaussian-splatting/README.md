...
> [!WARNING]
> **(team-sr fork) `conda env create` can fail at the pip step** with
> `The detected CUDA version (12.1) mismatches the version that was used to compile
> PyTorch (11.8)`. The env's torch is built for cu118 while `nvcc` on PATH is newer,
> so the CUDA submodules fail to build and the whole env creation aborts.
> Fix: pin `CUDA_HOME` to a matching toolkit and build the submodules by hand.
> ```bash
> conda activate surfel_splatting
...
> # verify
> python -c "import torch, diff_surfel_rasterization, simple_knn; print(torch.__version__, torch.version.cuda, torch.cuda.is_available())"
> ```
> (The exports are only needed at build time, not for training or rendering.)
## Training
...
```bash
--lambda_normal  # hyperparameter for normal consistency (paper: beta = 0.05)
--lambda_dist    # hyperparameter for depth distortion (paper: alpha = 1000 bounded / 100 unbounded)
...
> [!IMPORTANT]
> **(team-sr fork) Two fixes to the note above.**
> 1. The flag is `--lambda_dist`, **not** `--lambda_distortion` (upstream README typo).
>    The CLI name comes from the attribute in `arguments/__init__.py`, so
>    `--lambda_distortion` is rejected as an unrecognized argument.
> 2. The code default is `lambda_dist = 0.0` (a leftover from the 3DGS port), which
>    means **depth-distortion regularization is OFF unless you pass it explicitly** and
>    surfaces come out thick / blurry. Always pass the paper value:
>    ```bash
>    python train.py -s <scene> -m output/<name> --depth_ratio 0 --lambda_dist 100 --lambda_normal 0.05
>    #   unbounded / indoor room
>    #   --lambda_dist 1000 for a bounded, object-centric capture
>    ```
>    Note both terms are warmed up in `train.py`: distortion starts after iter 3000,
>    normal consistency after iter 7000.
...
Defaults follow the paper: `LAMBDA_DIST=100` (unbounded), `LAMBDA_NORMAL=0.05`,
`DEPTH_RATIO=0`. The runner reports the COLMAP registration rate and warns below 90%,
which is the first signal that a capture needs to be redone (motion blur, reflective
screens, or panning in place instead of walking around the subject).

Output mesh: `output/<scene>/train/ours_30000/fuse_unbounded_post.ply`
(`fuse_post.ply` in bounded mode) - vertex-colored PLY, floaters removed by
`--num_cluster`.

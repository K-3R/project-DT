...
"""
양팔 Franka workcell -- scene 정의.

Isaac Lab 2.2.1 에는 양팔 로봇 asset 이 없음 (양팔은 GR1T2 휴머노이드뿐).
대신 Franka 를 두 대 놓아 양팔 workcell 을 구성함.
robosuite 의 TwoArm 환경이 Panda 2대로 하는 것과 같은 구성.
...
주의
    이 파일 최상단은 isaaclab 를 import 하지 않음.
    AppLauncher 기동 전에는 omni.client 가 없어 import 자체가 실패함.
    실행 script 가 인자 정의(add_scene_args)를 앱 기동 전에 쓰므로
    isaaclab symbol 은 전부 build() 안에서 지연 import 함.

확정 preset 과 실측 수치는 PRESETS.md 참조.
"""
...
# 안 쓰는 큐브를 치워두는 자리 (테이블 밖, 바닥으로 떨어져 화면에서 사라짐)
PARK = (2.5, 2.5, 0.5)

# 테이블 원점에서 상판 몸통 중심까지의 offset (SeattleLabTable, 실측).
# 원점이 mount 쪽에 치우쳐 두 장을 그냥 나란히 놓으면 한쪽 다리가 튀어나옴.
# z 회전으로는 교정 불가, 거울 반사(scale y=-1)만 가능함. PRESETS.md 참조.
...
def build(a, with_camera=True):
    ...
    # 접촉 센서를 쓰려면 spawn 에서 contact reporter API 를 켜야 함
    # (Franka 기본값 False). self-collision 은 net force 판정의 오탐이 되므로
    # 끔 (기본값 True). replace 로 사본을 만들어 원본 cfg 오염을 방지함.
    ...
    class DualFrankaSceneCfg(InteractiveSceneCfg):
        ...
        # IK 추종이 좋은 강성 PD 판. Franka stack task 가 쓰는 것과 동일.
    ...
    # render decimation 중에도 순간 접촉을 놓치지 않음.
    ...
    if with_camera:
        ...
        # 손목 카메라 2대 (2026-08-11, 3view 전환)
        #   장착 위치/자세: Isaac Lab 공식 Franka stack task 의 wrist_cam
        #     (stack_ik_rel_visuomotor_env_cfg.py, panda_hand 부착, ros 규약)
        #   화각: LIBERO / robosuite eye_in_hand 를 따라 광각 (기본 75도).
        #     Isaac Lab 기본 aperture 20.955 는 약 47도라 훨씬 좁음.
    ...
    def usd_table(name, y, mirror):
        ...
        if mirror == 1:  # z축 180도. x offset 도 뒤집혀 어긋남
    ...
    def proc_table():
        """절차적 테이블. 원점이 곧 상판 중심이라 반전, mount 판 문제 없음."""
    ...
    # 테이블이 없으면 지면 자체가 작업면이 되어야 큐브가 안 떨어짐.

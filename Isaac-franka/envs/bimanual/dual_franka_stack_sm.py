#!/usr/bin/env python
# [ver] dual_franka_stack_sm.py 2026-08-05-r5  (training data: 20Hz camera + HDF5 v2)
r"""
양팔 Franka -- 흩어진 큐브를 하나의 탑으로 쌓는 정답(씨앗) 궤적 생성기.

텔레옵 없이 스크립트 상태기계로 만든다. GR00T 파인튜닝용 학습 데이터를
직접 출력한다 (카메라 프레임 포함, 20Hz). 정본 계획은
...
태스크
    ...
    N             --num-cubes 범위에서 에피소드마다 무작위 (기본 2~5)

    집기는 두 팔이 동시에 한다 (담당 큐브가 서로 반대편이라 동선이 안 겹친다).
    쌓는 자리는 하나뿐이므로 놓기만 한 번에 한 팔씩 -- Stack 이 lock 으로 조율한다.
    로봇 링크(link2~7)에 접촉력이 감지되면 그 에피소드는 실패로 버린다.


제어와 기록 주기
    물리 120Hz, 기록은 record-every(6) 스텝마다 = 20Hz.
    렌더는 매 스텝 수행한다 (일반 방식). 기록 시점에만 렌더하는 절약
    방식은 temporal 누적버퍼(AA/denoiser)에 이전 포즈가 섞여 팔 잔상이
    생기는 문제로 폐기했다 (FXAA 전환으로도 안 사라짐, 08-06 검증).

    주의: IK 가 제어하는 것은 panda_hand 원점이고, 실제 파지점(TCP)은
    거기서 손의 +z 로 tool_offset(0.1034) 만큼 나간 지점이다.
...
# bimanual_scene 은 최상단에서 isaaclab 를 import 하지 않게 만들어 두었다.
# (앱 기동 전에는 omni.client 가 없어 import 자체가 실패한다)
...
# 학습 데이터에 카메라 프레임이 반드시 들어가므로 항상 켠다
args.enable_cameras = True

app_launcher = AppLauncher(args)
simulation_app = app_launcher.app

# ---- 산출물 경로에 실행 시각을 붙인다 (여러 번 돌려도 안 덮이도록) ----
if args.stamp:
    # 컨테이너가 UTC 라 로컬시간 기준이면 9시간 이르게 찍힌다 -> KST 고정
...
# ================================================================== 3. 수학
DOWN_QUAT = (0.0, 1.0, 0.0, 0.0)  # x축 180도 회전. 그리퍼가 아래를 향한다
...
def canonical_quat(q):
    """쿼터니언 이중피복 제거: qw>=0 (w==0 이면 x>=0) 반구로 통일.

    q 와 -q 는 같은 회전이지만 기록에서 부호가 섞이면 학습 시
    per-component min_max 정규화가 망가진다 (검증에서 확인된 함정).
    """
...
def grasp_quat(yaw):
    ...
    큐브가 돌아가 있으면 그리퍼도 같이 돌려야 면을 문다.
    정사각 단면이라 90도 주기로 같으므로 회전량을 +/-45도 안으로 접는다.
    """
...
def sample_layout(rng, a, n, base_l, base_r):
    """구역 안에 큐브 n 개를 뿌린다. cube_1 이 탑의 바닥.

    제약
      - 바닥과 나머지는 base_clear 이상 (쌓는 중 간섭 방지)
      - 나머지끼리는 min_sep 이상 (그리퍼가 옆 큐브를 안 건드리게)
      - 각 큐브는 "가까운 팔"의 리치 안
      - 바닥은 양팔 모두 닿아야 한다 (누가 쌓을지 모르므로)
    """
...
class Stack:
    ...
    놓을 높이를 "바닥 + level x 4cm" 로 가정하지 않고 현재 탑의 실측 높이에서
    구한다. 앞 큐브가 조금 눌리거나 뜬 채로 놓이면 오차가 누적되기 때문이다.
    """
...
class Arm:
    ...
    def __init__(self, scene, key, sim, stack, queue, a):
        ...
        self.key = key
        self.stack = stack
        self.queue = list(queue)  # 이 팔이 담당할 큐브들
        self.cube_key = None
        self.state = REST if self.queue else FINISHED
        self.timer = 0  # 도달 후 체류 카운터
        self.elapsed = 0  # 현재 상태에 머문 스텝 (시간초과용)
        self.just_placed = False  # 이번 스텝에 한 큐브를 쌓아 올렸는가

        self.pick = None  # 파지 지점 (REST 에서 고정)
        self.pick_q = DOWN_QUAT
        self.place_q = grasp_quat(stack.yaw)  # 탑 각도에 맞춰 놓는다
        self.place_z = None  # 놓을 큐브의 목표 중심 z
        self.hold_off = a.grasp_z  # TCP-큐브중심 실측 간격 (CLOSE 에서 갱신)
        ...
        self.target_w = None  # hand 원점 목표 (월드)
        self.tcp_target_w = None  # 파지점 목표 (월드, 기록용)
        self.hold_here()  # 시작부터 IK 명령을 채워둔다
    ...
    def tcp_now(self):
        """지금 파지점의 월드 위치. hand 원점에서 손 +z 로 tool 만큼."""
    ...
    def aim(self, tcp_pos_w, quat_w=DOWN_QUAT):
        ...
        quat 은 canonical_quat 로 반구를 통일해 기록의 부호 일관성을 지킨다.
        """
    ...
    def hold_here(self):
        """지금 자세를 목표로 잡아 그 자리에 세운다.

        IK 컨트롤러는 set_command 없이 compute 하면 목표가 0 으로 잡혀
        팔이 루트 원점 쪽으로 끌려간다. 명령이 없는 구간에서 반드시 부를 것.
        """
        ep, eq = self.eef_pose_w()
        rp, rq = self.root_pose_w()
    ...
    def _reached(self, a):
        """목표에 dwell 스텝 이상 머물면 True. 시간초과도 True (안전장치)."""
        self.elapsed += 1
        if self.dist() < a.pos_tol:
            ...
            self.timer = 0
        # 접촉으로 막히면 거리가 영영 안 줄어든다 (예: 놓을 때 아래 큐브에 닿음)
    ...
    def step(self, scene, a):
        ...
        # 파지 지점은 REST 에서 고정한 값을 쓴다.
        # 매 스텝 큐브를 다시 읽으면 쥔 뒤에 큐브가 손을 따라와 목표가 도망간다.
        ...
        if s == ABOVE_PICK:
            ...
            if self.timer > a.dwell * 2:  # 손가락이 닫힐 시간
                # 큐브가 그리퍼 안에서 어디에 물렸는지 실측한다.
                # grasp_z 를 그대로 믿으면 놓을 때 누르거나 뜬다.
            ...
            self.aim(hover_pick, self.pick_q)  # 고정 지점이라 도망가지 않는다
            if self._reached(a):
                self.aim(standby, self.place_q)
                self._to(HOLD)

        elif s == HOLD:
            # 쥔 채 자기 쪽 바깥에서 대기. 쌓는 자리를 잡으면 진행한다.
            self.aim(standby, self.place_q)
            ...
            if self.timer > a.dwell * 2:
                # 먼저 수직으로만 빠진다. 바로 옆으로 가면 방금 쌓은 탑을 스친다.
...
# ================================================================== 7. 기록
class Recorder:
    """한 에피소드의 액션, 관측(카메라 포함), subtask 경계를 20Hz 로 모은다."""
    ...
    def mark_place(self):
        """기록 구간 사이에 발생한 놓기 완료를 최근 기록 스텝에 반영한다."""
...
def save_video(frames, path, fps):
    ...
    # yuv420p 명시: 플레이어 호환성 (기본값이 바뀌는 환경 대비), 코덱은 로그로 확인
    err = None
...
def collided_now(scene, th):
    """두 로봇의 link2~7 net 접촉력이 임계값을 넘었는가.

    history_length=6 이라 최근 6 물리스텝의 순간 접촉까지 커버한다.
    self-collision 은 spawn 에서 꺼 두었고 hand/finger 는 센서에서 제외했으므로
    여기 걸리는 접촉은 로봇-로봇 또는 로봇-테이블 충돌이다.
    """
...
def open_hdf5(path, a):
    """점진 저장용 HDF5 를 연다 (배치 시작 시 1회).

    에피소드를 램에 모아뒀다 끝에 쓰는 방식은 배치 210개 기준 피크가
    수십 GB 라 OOM 위험이 있어, 끝날 때마다 바로 쓰는 구조로 바꿨다 (08-06).
    """
    import h5py
...
def append_demo(grp, rec, steps, n):
    ...
    flush 까지 해 두면 도중에 프로세스가 죽어도 완료분은 파일에 남는다.
    """
    i = int(grp.attrs["total"])
    arr = rec.arrays()
    d = grp.create_group(f"demo_{i}")
    d.create_dataset("actions", data=arr["actions"])
    d.attrs["num_samples"] = int(arr["actions"].shape[0])  # 20Hz 스텝 수
...
def write_meta(meta_dir, a, stats, n_ok):
    """실행 메타를 기록한다. 에피소드마다 갱신되므로 진행 확인용으로도 쓴다."""
    import json
...
def reset_episode(scene, sim, rng, a, n, base_l, base_r, dt):
    """로봇을 초기 자세로, 큐브를 구역에 배치하고 카메라를 워밍업한다."""
    scene.reset()
    for r in (scene["robot_l"], scene["robot_r"]):
        q = r.data.default_joint_pos.clone()
        r.write_joint_state_to_sim(q, torch.zeros_like(q))
        # PD 목표도 초기 자세로 재설정. 상태만 리셋하면 안정화 구간 동안
        # 이전 에피소드의 마지막 IK 목표로 팔이 끌려간다 (충돌 중단 뒤
        # 다음 에피소드가 저공 자세로 시작하던 문제, 08-06 검증에서 발견)
    ...
    # 물리 안정화 + 카메라 워밍업: 리셋 직후에는 이전 에피소드 장면이
    # temporal 누적버퍼에 남아 첫 프레임에 흐리게 비친다 (프레임당 감쇠
    # ~0.86 실측). 30프레임이면 ~1% 잔여가 보여서 90프레임을 흘려보낸다
    for _ in range(90):
...
def assign_cubes(scene, n, base_l, base_r):
    """cube_1 을 뺀 나머지를 베이스가 가까운 팔에게 배정한다."""
    q_l, q_r = [], []
...
def run_episode(scene, sim, cam, a, ep, n, base_l, base_r, rng, dt):
    ...
    while step < limit and not (arm_l.state == FINISHED and arm_r.state == FINISHED):
        arm_l.step(scene, a)
        arm_r.step(scene, a)
        # 놓기 완료가 기록 스텝 사이에 발생해도 잃지 않도록 래치한다
        ...
        if step % max(a.record_every, 1) == 0:
            ...
            if placed_since_record:
                rec.mark_place()
                placed_since_record = False
            # 접촉 검사: history=6 이 기록 사이 물리스텝을 커버한다
...
def main():
    ...
    # 점진 저장: 파일을 먼저 열고, 성공 에피소드는 즉시 쓰고 램에서 버린다
    h5f = grp = None
    if args.out:
    ...
    for ep in range(args.demos):
        ...
        del rec  # 프레임 버퍼 즉시 해제 (RAM 을 에피소드 1개분으로 유지)
        if meta_dir:
            write_meta(meta_dir, args, stats, n_ok)  # 매 에피소드 갱신 = 진행 확인용

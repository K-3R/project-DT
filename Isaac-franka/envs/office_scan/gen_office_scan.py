#!/usr/bin/env python
# ======================================
# File: gen_office_scan.py
# ======================================
# Sanghyeok Park, SSL undergraduate
# Edit 2026-08-31
# ======================================
# [ver] gen_office_scan.py 2026-08-31-r2  (ascii-only console/comments)
# r2: env 격리 -- envs/office/gen_office.py 의 사본에 scene import 만
#     office_scan_scene 으로 교체 (구 r1 = _overlay runpy wrapper).
#     Track B 용: scan 배경 데모 생성 -> 재학습 = 인도메인 행
#     (task/상태기계/기록 형식 = office 동일, 변환기도 공용)
r"""
사무실 책상 task -- 마커를 연필꽂이에 꽂는 씨앗 궤적 생성기.

task
    책상에 눕혀 흩어진 마커(지름 2.6cm, 길이 14cm) 1~3개를 연필꽂이에
    세워 꽂음. 연필꽂이는 왼쪽 모니터 앞이라 왼팔만 닿음.

    왼쪽 마커   왼팔이 집어서 세워 꽂음
    오른쪽 마커 오른팔이 집어 공중에서 왼팔에 넘김 (mid-air handover,
               사용자 선택) -> 왼팔이 세워 꽂음

handover 기하
    오른팔이 마커를 수평(축=+x)으로 들고 handover 지점에 대기.
    grip 은 중심에서 +GRIP_OFF (긴 쪽이 -x). 왼팔이 -x 쪽 끝을 잡으면
    두 손목이 마커 축 방향으로 2*GRIP_OFF 떨어져 충돌하지 않음.
    왼팔이 닫은 뒤 오른팔이 열고 물러남.

세워 꽂기
    왼팔 grip 은 항상 "중심-GRIP_OFF, 긴 쪽 +hand-x" 로 통일됨
    (직접 집기/handover 모두). 손목을 world-y 로 +90도 돌리면(ERECT)
    hand-x 가 아래를 향해 마커가 수직으로 매달림. 연필꽂이 위에서
    내려 놓으면 벽이 guide 가 되어 들어감.

기록 (1번 환경과 동일 규약 + 3 view)
    물리 120Hz + 매 step render, 기록 20Hz (6 step 마다), 카메라 3 view 256x256
    (ego_view + left/right_wrist_view -- docs/threeview_camera_upgrade.md,
    key 이름/순서는 변환기 VIDEO_KEYS 와 정확히 일치해야 함),
    action 16차원 [Lpos3 Lquat4 Lgrip1 | R 동일] = world 절대 TCP 명령(qw>=0),
    state joint 18, HDF5 점진 저장 (성공분만, flush), 충돌은 실패 처리.

사용
    CUDA_VISIBLE_DEVICES=1 isaaclab.sh -p gen_office_scan.py --headless \
...
if args.stamp:
    # container 가 UTC 라 KST 로 고정함 (1번 환경과 동일)
...
# 불가 -> IK 가 팔을 휘둘러 책상에 충돌함 (영상으로 확인).
# 그래서 "아래보기"를 base 기준으로 재정의함:
...
# 운반 자세 (팔끼리 교차 방지를 위해 좌우가 거울):
#   handover 에서 각 팔이 마커의 "자기 쪽" 끝을 잡아야 팔이 안 겹침
#   (3차 실행에서 교차 설계로 팔끼리 충돌 -> 거울 규약으로 수정).
#   왼팔  CARRY_L = Rz(-90)*DOWN: hand-x = +y. 마커의 -y 쪽 끝을 잡고
#         긴 쪽(+hand-x = +y)이 오른팔 방향 -> ERECT_L 에서 아래로 감
#   오른팔 CARRY_R = Rz(+90)*DOWN: hand-x = -y. +y 쪽 끝을 잡음
# 세우기(왼팔만): ERECT_L = Rx(-90)*CARRY_L -> +hand-x(+y) 가 아래.
#   hand-z 는 -y 가 되어 손 몸통이 TCP 의 +y 쪽(중앙, 빈 공간)에 섬.
...
def grasp_quat(theta):
    ...
    theta 를 (-90,90] 로 접으므로 손목 비틀림이 90도를 넘지 않음.
    """
...
def marker_axis(scene, key):
    """마커 축의 world 방향각 (cylinder 축 = body z). (-90,90] 로 접음."""
    w, x, y, z = scene[key].data.root_quat_w[0].cpu().numpy().tolist()
    # (0,0,1) 을 quaternion 으로 회전한 결과의 xy 성분
    ax = 2.0 * (x * z + w * y)
    ay = 2.0 * (y * z - w * x)
...
def collided_now(scene, th):
    """link2~7 net 접촉력 임계값 초과 여부 (history 6 이 기록 사이 cover)."""
...
class Arm:
    ...
    def hold_here(self):
        ...
        # label 은 TCP 로 기록함. hand 원점을 넣으면 정지 팔의 action label 이
        # tool_offset(0.1034) 만큼 z 로 밀림 (env1 0.4절 결함의 상속 --
        # env2 는 생성 전이라 여기서 바로잡음. IK 명령은 hand 기준 그대로)
    ...
    def apply(self):
        ...
        ep, eq = self.eef_pose_w()
        rp, rq = self.root_pose_w()
        # PhysX Jacobian 은 "world" frame 기준인데 IK 오차는 "root" frame 에서
        # 계산함. base 가 z180 으로 돌아 있는 이 scene 에서 회전을 안 되돌리면
        # 방향이 반전돼 팔이 엉뚱한 곳으로 감 (2차 실행에서 실측 확인).
        # 공식 구현과 동일한 변환: task_space_actions.py 의 jacobian_b.
        # 1번 환경은 base 가 identity 라 생략해도 우연히 맞았던 것임.
...
class Program:
    ...
    이 task 는 팔 둘이 교대로 움직이는 순차 흐름이라 (handover 순간만
    맞물림), 팔별 상태기계 대신 primitive 목록이 단순하고 안전함.
    각 primitive: init(1회) -> done() 이 dwell 동안 참 -> 다음.
    state-timeout 이 지나면 경고 후 강제 전진함 (1번 환경과 동일 안전장치).
    """
    ...
    def move(self, name, arm, pos, quat=DOWN_QUAT):
        """pos/quat 는 값이거나 () -> 값 함수 (runtime 계산용).

        완료 = 위치 AND 자세 도달. 자세를 안 보면 제자리 회전(세우기 등)이
        즉시 통과돼 다음 동작이 회전 중에 시작됨 (실측: 하강 중 세워짐).
        """
    ...
    def gripper(self, name, arm, val, wait):
        ...
        # 손가락이 움직일 시간만 줌 (조건 없음)
        self.add(name, init, lambda: True, dwell=wait)

    def step(self):
        """물리 step 마다 호출. program 이 끝나면 True."""
        if self.done:
            return True
        if self.i < 0:
            self._advance()
...
def build_program(a, scene, arm_l, arm_r, order, bases):
    """episode program 생성. order = [(marker_key, need_handover)].

    부호 규약 (DOWN_QUAT = Ry180 기준, 거울 운반 규약):
      grasp_quat(th) 의 hand-x = -u(th). 두 팔 모두 grip = 중심 + off*u.
      reorient 로 왼팔은 CARRY_L(hand-x=+y), 오른팔은 CARRY_R(hand-x=-y).
        회전 후 물리 grip 위치: 왼팔 = 중심의 -y 쪽 / 오른팔 = +y 쪽
        = handover 에서 각자 자기 진영의 끝을 잡음 (팔 교차 없음).
      왼팔의 긴 쪽은 +hand-x(+y) -> ERECT_L 에서 아래로 늘어짐.
    """
    prog = Program(a)
    ...
    hang = half + off  # ERECT 후 grip 아래로 늘어지는 길이
    bb = {}

    def pick_info(key, sign):
        """집기 좌표: 중심 +- off 를 축 방향으로. reset 후 고정값을 씀."""
    ...
    # 삽입 slot: 매번 정중앙에 꽂으면 3개째부터 먼저 선 마커와 부딪힘
    # (실측: 2개 성공 후 3개째 실패). 통 안 네 구석을 순서대로 씀.
    ...
    def insert(tag, l_off, slot_i):
        """왼팔이 (grip = 중심 -y 쪽, 긴쪽 +y) 로 쥔 마커를 세워 꽂음.

        l_off = 왼팔 grip 의 중심 offset. 직접 집기는 grip_off, handover 는
        handover_off 라 hang(grip 아래로 늘어지는 길이)이 경로마다 다름.
        slot_i = 삽입 순번 (slot 선택).
        """
        hang2 = half + l_off
    ...
    for idx, (key, handover) in enumerate(order):
        ...
        if not handover:
            ...
            # ---- 오른팔이 집어 handover -> 왼팔이 꽂음 ----
            pick(f"{tag}:R", arm_r, key, CARRY_R, 0.26)
            # handover: 마커 축 = y, 중심이 hoz. 오른팔 grip 은 +y 쪽(자기 진영)
            prog.move(
                f"{tag}:r_handover",
                arm_r,
...
def open_hdf5(path, a):
    """점진 저장용 HDF5 (batch 시작 시 1회). RAM 상수 + crash 내성."""
    import h5py
...
def forbidden_rects(a):
    ...
    return [
        ...
        # 연필꽂이 주변은 넓게 비움: gripper 손 폭(~9cm) + 파지 offset 때문에
        # 가까이 놓인 마커는 손이 연필꽂이에 걸려 못 집음 (실측).
        # 통이 2배(17cm)로 커져서 금지 반경도 넓힘
...
def reset_episode(scene, sim, rng, a, n, dt):
    """로봇/마커 reset + 물리 안정화 + render warmup (1번 환경 규약)."""
    scene.reset()
...
def run_episode(scene, sim, cam, wcam_l, wcam_r, a, ep, n, bases, rng, dt, hd_cams=()):
    ...
    # (handover 경유), 나머지는 왼팔 직접. 왼팔 reach 밖이면 무조건 handover.
    order = []
    bl = np.array(bases[0])
    for i in range(1, n + 1):
        p = scene[f"item_{i}"].data.root_pos_w[0].cpu().numpy()
    ...
    order.sort(key=lambda t: t[1])  # 직접 집는 것 먼저 (실패 시 빨리 끝남)
    print(
        f"  markers n={n}  "
    ...
    if a.video_dir:
        # ego + 손목 2 view 전부 저장 (손목 화각 육안 판정용)
...
def main():
    ...
    # 손목 카메라는 panda_hand 에 OffsetCfg 로 붙어 있어 pose 설정 불필요
    ...
    for ep in range(args.demos):
        ...
        del rec  # frame buffer 즉시 해제 (RAM 상수 유지)
        if meta_dir:

...
# Edit 2026-08-31
# ======================================
# [ver] eval_office_scan.py 2026-08-31-r3  (ascii-only console/comments)
# r3: env 격리 -- envs/office/eval_office.py 의 사본에 scene import 만
#     office_scan_scene 으로 교체 (구 r2 = _overlay runpy wrapper).
#     배경 도메인 갭 측정: task/로봇/카메라/판정 = office 동일
r"""
사무실 마커 task 폐루프 평가 harness (env2).

finetune 된 GR00T checkpoint 를 추론 server 에 띄워 두고, 이 script 가
생성기(gen_office_scan.py)와 같은 scene/random 배치에서 정책을 굴려
...
사용 (server 를 먼저 띄운 뒤; run_server_finetuned.sh CKPT=ckpt_office_3view)
    CUDA_VISIBLE_DEVICES=1 isaaclab.sh -p eval_office_scan.py \
...
g.add_argument("--seed", type=int, default=1000, help="eval seed (train used 100-300)")
g.add_argument(
    "--dry-run",
    action="store_true",
...
# ================================================================== 3. 수학
# 생성기(gen_office_scan.py)와 동일한 규약. 생성기는 script 라

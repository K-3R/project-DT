...
# Edit 2026-08-31
# ======================================
# [ver] run_eval_office.sh 2026-08-31-r2
# r2: 원샷 통합 -- server 자동 기동/종료 흡수 (구 r1 = client 전용, server
#     수동 기동 필요했음). mode 3종: 기본 / DRY_RUN / EXTERNAL_SERVER.
#     로그 prefix [eval-office] 로 분화 (구 [eval] 은 bimanual 과 충돌)
# =============================================================================
# 사무실 마커 task 원샷 평가:
# server 자동 기동 -> port 대기 -> eval -> server 자동 종료 -> summary 출력
#
# 사용 (host, gr00t_sh 활성화 상태, clone root 기준):
#   SERVER_GPU=2 CLIENT_GPU=1 CKPT=/data1/huggingface/sslunder54/checkpoints/office_3view \
#     nohup bash Isaac-franka/envs/office/run_eval_office.sh \
#     > out/eval_office.log 2>&1 &
...
#
# 산출물: container /root/project/out/eval_office/<KST stamp>/
#   (host <클론루트>/out/eval_office/... 로 보임)
#
# 중단: pkill -f run_eval_office.sh
...
if [ "$DRY_RUN" = "1" ] && [ "$EXTERNAL_SERVER" = "1" ]; then
    echo "[eval-office] ERROR: DRY_RUN and EXTERNAL_SERVER are exclusive"
    exit 1
fi
# 공용 server 규약: GPU 반드시 명시 (조용한 기본값 금지. DRY_RUN 도
# container 에서 Isaac 을 기동하므로 GPU 점유 발생)
if [ -z "$CLIENT_GPU" ]; then
    echo "[eval-office] ERROR: set CLIENT_GPU explicitly (shared server), e.g. CLIENT_GPU=2 ..."
    exit 1
fi
if [ "$DRY_RUN" = "0" ] && [ "$EXTERNAL_SERVER" = "0" ]; then
    if [ -z "$SERVER_GPU" ]; then
        echo "[eval-office] ERROR: set SERVER_GPU explicitly (shared server)"
        exit 1
    fi
    # CKPT 도 GPU 처럼 명시 필수 (조용한 기본값 = 엉뚱한 ckpt 평가 함정)
    if [ -z "$CKPT" ]; then
        echo "[eval-office] ERROR: set CKPT explicitly, e.g."
        echo "  CKPT=/data1/huggingface/sslunder54/checkpoints/office_3view"
        exit 1
    fi
    if [ ! -d "$CKPT" ]; then
        echo "[eval-office] ERROR: checkpoint not found: $CKPT"
        exit 1
    fi
    if [ "${CONDA_DEFAULT_ENV:-}" != "gr00t_sh" ]; then
        echo "[eval-office] WARNING: conda env is '${CONDA_DEFAULT_ENV:-none}' (server needs gr00t_sh)"
    fi
fi
...
echo "[eval-office] gpu=$CLIENT_GPU episodes_per_n=$EPISODES_PER_N port=$PORT seed=$SEED"
echo "[eval-office] markers=$NUM_MARKERS steps_per_marker=$STEPS_PER_MARKER"
...
mkdir -p "$OUT_ROOT"
CLIENT_LOG="$OUT_ROOT/eval_office_${STAMP}_client.log"

# ---- mode: EXTERNAL_SERVER (떠 있는 server 재사용) ----
if [ "$EXTERNAL_SERVER" = "1" ]; then
    if ! nc -z localhost "$PORT" 2>/dev/null; then
        echo "[eval-office] ERROR: no server on port $PORT (EXTERNAL_SERVER=1)"
        exit 1
    fi
    echo "[eval-office] using external server on port $PORT"
    # 중단 시 container 안 client zombie 정리 (정상 종료 후에는 no-op)
    trap 'docker exec -u 0 -e TERM=xterm "$CONTAINER" bash -lc "pkill -9 -f \"[e]val_office.py\"" 2>/dev/null || true' EXIT
else
    # ---- mode: 기본 (server 자동 기동/종료) ----
    # port 선점 검사: 이미 물려 있으면 남의 server 일 수 있으니 죽이지 않고 거부
    if nc -z localhost "$PORT" 2>/dev/null; then
        echo "[eval-office] ERROR: port $PORT already in use"
        echo "  reuse it with EXTERNAL_SERVER=1, or check: pgrep -af \"inference_service.py.*--port $PORT\""
        exit 1
    fi
    SERVER_LOG="$OUT_ROOT/eval_office_${STAMP}_server.log"
    echo "[eval-office] ckpt=$CKPT"
    echo "[eval-office] server gpu=$SERVER_GPU stamp=$STAMP"
    ...
    cleanup() {
        ...
        echo "[eval-office] stopping server (pid $SERVER_PID)..."
        ...
        docker exec -u 0 -e TERM=xterm "$CONTAINER" bash -lc \
            "pkill -9 -f '[e]val_office.py'" 2>/dev/null || true
        echo "[eval-office] cleanup done"
    }
    trap cleanup EXIT

    # server 대기: port 가 열리거나, server 가 죽거나, timeout
    echo "[eval-office] waiting for server on port $PORT (ckpt load takes ~minutes)..."
    waited=0
    while ! nc -z localhost "$PORT" 2>/dev/null; do
        if ! kill -0 "$SERVER_PID" 2>/dev/null; then
            echo "[eval-office] ERROR: server died during startup -- see $SERVER_LOG"
            exit 1
        fi
        sleep 3
        waited=$((waited + 3))
        if [ "$waited" -ge 600 ]; then
            echo "[eval-office] ERROR: server not up after ${waited}s -- see $SERVER_LOG"
            exit 1
        fi
    done
    echo "[eval-office] server up after ~${waited}s"
fi
...
echo "[eval-office] eval finished rc=$rc"
LATEST=$(ls -dt "$HOST_OUT"/*/ 2>/dev/null | head -1)
if [ -n "$LATEST" ] && [ -f "$LATEST/summary.json" ]; then
    echo "[eval-office] summary: $LATEST/summary.json"

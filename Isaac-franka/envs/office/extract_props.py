...
r"""
office.usd 안의 소품을 독립 USD 로 추출함 (asset 다운로드 불필요).

Isaac asset 의 Environments/Office/office.usd 에 모니터/키보드/마우스/
서류트레이/펜꽂이 같은 사무실 소품이 전부 들어 있음. 다만 방 안에 놓인
상태라 (1) 방 좌표의 위치/회전이 박혀 있고 (2) 크기를 알 수 없음.

소품마다 작은 wrapper USD 생성:
    - 원본 prim 을 reference 로 참조 (재질/texture 그대로 따라옴)
    - 방 좌표의 변환 제거 (identity)
    - xy 중심 + z 바닥이 원점에 오도록 재정렬
결과물은 scene 에서 일반 asset 처럼 UsdFileCfg 로 사용 가능함.
...
출력
    --out 아래에 <이름>.usd. 마지막에 실제 치수(회전 제거 후)를 표로 출력함.
    그 수치로 office_scene.py 의 배치를 확정함.
"""
...
parser.add_argument(
    "--out",
    default="/root/project/Isaac-franka/envs/office/props",
...
# ---- 마커 wrapper mode (--marker-src 지정 시 이것만 실행) -----------------
parser.add_argument(
    "--marker-src",
    default="",
...
# 기본 추출 목록. (이름, office.usd 안의 prim, 원점 mode, 물리)
# 회전이 덜 섞인 instance 선택 (bbox 실측으로 판단).
#
# 원점 mode
#   bottom = xy 중심 + z 바닥 (책상 위에 z=0 으로 그냥 놓는 물건)
#   center = 기하 중심 (회전시켜 쓰는 물건. 모니터 패널을 세로로 피벗할 때
#            원점이 바닥이면 90도 회전 시 옆으로 드러누움)
#
# 물리
#   none  = 시각 전용 (정적 배경 소품)
#   rigid = RigidBodyAPI + MassAPI + mesh CollisionAPI 부착.
#           Isaac Lab 의 UsdFileCfg 는 rigid_props 를 "수정"만 하므로
#           USD 에 schema 가 없으면 "Failed to find a rigid body" 로 죽음.
DEFAULT_PICKS = [
    # 정적 맥락 (팔이 닿지 않는 자리)
    # monitor_pc = 스탠드 포함, monitor_panel = 패널만 (피벗 세로 배치용)
    ...
    # 본체(타워)는 office.usd 에 없음. SM_PC* 는 전부 화면 일체형(아이맥형)이라
    # 책상 옆 타워로 쓸 수 없어 scene 에서 primitive 로 생성함.
    ("papertray", "/Root/SM_PaperTray10", "bottom", "none"),
    # 파지 후보 (gripper 개구 8cm 안에 드는지 치수로 판단할 것)
    # 마우스는 원본이 뒤집혀 있어 x 로 180도 돌려 세움 (렌더에서 확인)
...
def deinstance(stage):
    """instanceable prim 전부 해제.

    office.usd 의 소품은 instanceable 로 authoring 돼 있어, 그대로 두면
    Isaac Lab 이 RigidBody/Collision API 를 못 붙임 ("Could not perform
    modify_rigid_body_properties ... instanced prim paths" 경고 뒤
    "Failed to find a rigid body" 로 죽음). 로컬 layer 에서 해제함.
    """
    for _ in range(6):  # 상위를 풀면 하위가 새로 드러나므로 반복함
        changed = False
...
def add_physics(stage, root_prim):
    """rigid body schema 부착 (파지 대상 물건용).

    Isaac Lab 의 UsdFileCfg 는 rigid_props/mass_props 를 "수정"만 하므로,
    시각 mesh 만 있는 USD 를 그대로 쓰면 초기화에서
    "Failed to find a rigid body" 로 죽음. 여기서 미리 authoring 함.
    충돌은 볼록껍질 근사 (작은 문구류라 이 정도로 충분함).
    """
...
# scene 의 box 마커와 동일해야 함 (office_scene.py make_item 의 physics_material)
MARKER_FRICTION = (2.0, 1.8)  # static, dynamic (combine=max)


def extract_marker(src, meta):
    """펜류 asset 을 '실물 비주얼 + 보이지 않는 box 충돌체' wrapper 로 추출함.

    실물 펜/마커는 물리 파지 불가 (단면이 얇아 손끝이 상판에 닿고,
    원형 단면은 26mm 원기둥도 미끄러짐 -- 둘 다 실험으로 확정). 그래서
    충돌/질량은 검증된 box 마커 치수(--marker-fit) 그대로 두고, 비주얼만
    실물 asset 을 그 크기로 비균등 scale 해 입힘.
    물리 schema 와 마찰 재질(2.0/1.8 combine=max)은 USD 에 직접 authoring
    함 -- Isaac Lab 의 UsdFileCfg 는 physics_material 을 못 받음.
    원점 = center (CuboidCfg 와 동일 규약), 긴 축 = z (--marker-prerot).
    충돌체는 purpose=guide 라 렌더에 안 보이지만 PhysX 는 충돌로 파싱함.
    """
    ...
    # 1) 변환 op 는 한 번만 만들고 값만 갱신함 (같은 op 를 다시 Add
    #    하면 USD 가 중복으로 거부함). 나열 [T, S, R] = R -> S -> T 적용.
    #    초기값 항등이므로 첫 실측은 prerot 만 반영된 원본 치수임.
    ...
    # 2) 비주얼을 fit 치수로 비균등 scale + center 원점 정렬
    #    (scale 은 prerot 이후 frame 에서 실측했으므로 R 다음 S 순서가 맞음)
    ...
    # 3) 물리: 강체 + 보이지 않는 box 충돌체. 비주얼 mesh 에는 충돌 미부착
    #    (볼록껍질이면 둥근 단면이 되살아나 미끄러짐 재발)
    ...
    # 4) 마찰 재질을 충돌체에 physics binding (scene box 마커와 동일 값)
...
def add_vec_op(geom, kind):
    """translate/scale op 를 기존 attr 의 정밀도에 맞춰 추가함.

    reference 로 끌려온 원본 prim 에 xformOp:scale 등이 double3 로 이미
    authoring 돼 있으면, 기본 정밀도(float)로 Add 할 때 Tf 예외 발생
    (SM_MarkerBlue 실측). (op, 벡터 생성자) 반환."""
...
def apply_prerot(geom, spec):
    ...
    방 좌표 변환을 지우면 그대로 드러나므로 여기서 교정함.
    회전을 먼저 넣고 그 뒤에 크기/원점을 재야 정렬이 안 어긋남.
    정밀도는 기존 attr 에 맞춤 (add_vec_op 와 같은 이유)."""
    if not spec:
        return
...
def extract(src, name, prim_path, out_dir, meta, mode, physics, prerot=""):
    """소품 하나를 wrapper USD 로 저장, (치수, 경로) 반환."""
    out_path = os.path.join(out_dir, f"{name}.usd")
    if os.path.exists(out_path):
        os.remove(out_path)
    stage = Usd.Stage.CreateNew(out_path)
    # 원본과 단위/업축을 맞춤 (안 맞으면 100배 크기로 들어옴)
    UsdGeom.SetStageUpAxis(stage, meta["up"])
    UsdGeom.SetStageMetersPerUnit(stage, meta["mpu"])

    # 구조 2단 분리.
    #   /Prop       변환 없이 비워 둠. Isaac Lab 이 spawn 시 자기 translate/
    #               orient/scale 을 여기에 authoring 하기 때문.
    #   /Prop/Geom  참조 + 정렬 변환. 한 단계 아래라 안 덮어써짐.
    # 정렬을 /Prop 에 두면 Isaac Lab 이 그것을 지워, 패널이 원본 로컬 원점
    # (화면 구석 등) 기준으로 놓여 회전 시 엉뚱한 곳에 붙음 (실측 확인).
    root = UsdGeom.Xform.Define(stage, "/Prop")
    prim = root.GetPrim()
    stage.SetDefaultPrim(prim)
    geom = UsdGeom.Xform.Define(stage, "/Prop/Geom")
    ...
    # 1) 방 좌표의 변환 제거 + 사전 회전.
    #    로컬 layer 의 xformOpOrder 가 reference 를 이김
    ...
    #    center = 회전시켜 쓰는 물건 (패널을 세로로 피벗해도 안 드러누움)
    #    USD 는 나열 순서의 뒤쪽이 먼저 적용되므로 [translate, rotate] 로 둠
    ...
    # 3) 물리 schema (파지 대상만). 크기를 다 잰 뒤에 부착함
    if physics == "rigid":
    ...
    stage.GetRootLayer().Save()
    # zmin/zmax 도 같이 반환. 원점 규약(bottom 이면 zmin=0,
    # center 면 zmin=-h/2)을 눈으로 확인해야 배치 버그를 막음
...
def main():
    ...
    # 마커 wrapper mode: 이것만 실행 후 종료 (기존 추출물은 안 건드림)
    if args.marker_src.strip():
        ...
        if p is None or not p.IsValid():
            ...
            print("      check the prim path inside the source usd")
            return
        extract_marker(src, meta)
        return
    rows = []
    # (이름, prim, 원점, 물리[, 사전회전]) -- 사전회전은 생략 가능
...
if __name__ == "__main__":
    # 주의: finally 의 os._exit 는 예외 traceback 출력을 삼킴 --
    # 여기서 직접 찍고 rc 를 넘겨야 실패가 조용히 안 사라짐
    rc = 0
    try:
        main()
    except Exception:
